...
fn_scores_ens <- function(ens, y, skip_evals = NULL, scores_ens = TRUE){
  ...
  ###-----------------------------------------------------------------------------
  ###Output
  #...scores_ens...Data frame containing (n x 6 data frame):
  ...
  if(is.element("lgt", colnames(scores_ens))){
    ...
    # Quantiles are not included: Calculate corresponding via quantile function
      ...
      # Choose type 8, as suggested in ?quantile (and bias observed for LP for default)
...
#### DISTR: Evaluation of distributional forecasts ####
# Function for prediction based on the distributional parameters #
fn_scores_distr <- function(f, y, distr = "tlogis", 
                            n_ens = 20, skip_evals = NULL){
  ###-----------------------------------------------------------------------------
  ###Input
  #f............Parameters of forecast distribution (n x n_par matrix)
  #y............Observations (n vector)
  #distr........Parametric distribution ("tlogis", "norm")
  ...
  # Input check
  if(is.element(distr, c("tlogis", "norm")) & any(f[,2] < 0)){ print("Non-positive scale forecast!") }
  ...
  if(distr == "tlogis"){ # truncated logistic
    ...
    # Calculate length of ~(n_ens-1)/(n_ens+1) % prediction interval
    if(is.element("lgt", colnames(scores_pp))){
      scores_pp[["lgt"]] <- qnorm(p = n_ens/(n_ens + 1), 
                                  mean = f[,1], 
